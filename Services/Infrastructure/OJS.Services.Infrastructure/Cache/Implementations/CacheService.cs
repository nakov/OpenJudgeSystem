...
public class CacheService : ICacheService
{
    ...
    public Task<T> Get<T>(string cacheId, Func<Task<T>> getItemCallback, int cacheSeconds, int slidingExpirationSeconds = 0)
        => this.Get(
            cacheId,
            getItemCallback,
            this.datesService.GetAbsoluteExpirationBySeconds(cacheSeconds),
    ...
    private async Task<T> Get<T>(string cacheId, Func<Task<T>> getItemCallback, DateTime absoluteExpiration, TimeSpan slidingExpirationOffset)
        => await this.resilienceStrategiesService.ExecuteRedisWithCircuitBreaker(
            async _ =>
            {
                ...
                if (slidingExpirationOffset != TimeSpan.Zero)
                {
                    options.SetSlidingExpiration(slidingExpirationOffset);
                }
